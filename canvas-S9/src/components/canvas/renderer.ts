...
const drawBordure = (
    ...
    ctx.beginPath()
    ctx.fillStyle = color
    ctx.strokeRect((x + renderProps.pos.x) * renderProps.scale,
    (y + renderProps.pos.y) * renderProps.scale, 
    ctx.canvas.width * renderProps.scale, ctx.canvas.height * renderProps.scale)
}

export const render =
  (ctx: CanvasRenderingContext2D, props: RenderProps) => (state: State) => {
    clear(ctx)
    drawBordure(ctx, props, { x: 0, y: 0 }, COLORS.BLUE)
    state.pos.map((c) =>
      drawCirle(
        ctx,
        props,
